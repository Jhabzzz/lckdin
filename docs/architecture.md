...
| `ai_global_usage` | project-wide Gemini call counter for `ai-coach` + `analyze-journal` combined, per **America/Los_Angeles** day (Gemini's daily reset). Capped at 18/day (free tier is 20). RLS on with no policies; service role only, via `ai_global_take(limit)` (atomic; returns the day counted, or null when spent) and `ai_global_refund(day)`. **Fails closed**: if the counter errors, the call is refused |
...
|---|---|---|---|
| `analyze-journal` | Snap & Track in dashboard | user JWT | Sends a journal photo + rule list to Gemini, returns per-rule done/not-done. Max 3 calls per user per UTC day (`ai_daily_usage`); 429 `daily_limit` / `ai_recharging` when capped. Also capped project-wide at 18 Gemini calls per Pacific day across both functions (`ai_global_usage`, fails closed); 429 `ai_recharging` when spent |
| `ai-coach` | AI Coach in dashboard: once per day on load, plus the Refresh button (never on saves) | user JWT | Returns today's cached briefing from `ai_coach_briefings` if one exists (no Gemini call). Otherwise, or with `refresh: true`, it sends recent history + root-cause stats to Gemini and caches the result. Every Gemini call counts toward 3 per user per UTC day (`ai_daily_usage`); 429 `daily_limit` / `ai_recharging` when capped. Also capped project-wide at 18 Gemini calls per Pacific day across both functions (`ai_global_usage`, fails closed); 429 `ai_recharging` when spent |
