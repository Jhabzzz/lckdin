...
| `ai-coach` | AI Coach in dashboard | user JWT | Sends recent history + root-cause stats to Gemini, returns coaching text. Max 3 calls per user per UTC day (`ai_daily_usage`); 429 `daily_limit` / `ai_recharging` when capped |
