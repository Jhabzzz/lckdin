create or replace function public.use_pivot(p_day_number int)
returns json
language plpgsql
security definer
...
begin
  ...
  if v_log.is_pivot then
    return json_build_object('success', true, 'already_pivoted', true, 'pivots_used', v_pivot_count, 'pivots_max', 3);
  end if;

  if v_pivot_count >= 3 then
    return json_build_object('success', false, 'error', 'No pivots remaining', 'pivots_used', v_pivot_count, 'pivots_max', 3);
  end if;

  update public.daily_logs set is_pivot = true
    where user_id = v_user and day_number = p_day_number;

  return json_build_object('success', true, 'pivots_used', v_pivot_count + 1, 'pivots_max', 3);
end;
$$;

grant execute on function public.use_pivot(int) to authenticated;
